...
class W2V_ModelTasks:
    
    def __init__(self, model_type, model_dir, corpus_file):
        print(f"W2V_ModleTasks initialised with \n - model type: {model_type}\n - corpus file: {corpus_file}\n - target directory for models: {model_dir}")
    ...
    # Create W2V model
    #   
    def create_w2v_model(self, model_type, vector_size, window_size, min_count):
        current_date    = datetime.now().strftime('%Y%m%d')
        model_base_name = "w2v_"+current_date + "_sg"+str(model_type) +"_v"+str(vector_size)+"_w"+str(window_size)
        ...
        w2v = Word2Vec(self.sentences, sg=model_type, vector_size=vector_size, window=window_size, workers=4, epochs=10, min_count=min_count)
