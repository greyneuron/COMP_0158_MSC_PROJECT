...
class W2V_AdaBoost:
    ...
    def run(self, debug=False):
        ...
        if(debug):
            logfile_name = self.output_dir+timestamp+'_adab_clustering_results.txt'
            log = open(logfile_name, "a")
            
        #print(f"training indices {selected_indices}\nX train:\n {X_train}")
        
        base_classifier = DecisionTreeClassifier(max_depth=self.depth)
        #base_classifier.fit(X_train, Y_train)

        classifier = AdaBoostClassifier(estimator=base_classifier)
        classifier.fit(X_train, Y_train)
        print(f" - adaboost classifier - training")
